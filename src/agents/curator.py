...
face as every other agent, so the engine can dispatch to it without knowing
it works nothing like the companion.
"""
...
class CuratorAgent:
    ...
    async def find_users_needing_curation(self) -> list[str]:
        """discovery stays on the service; surfaced here so the scheduler
